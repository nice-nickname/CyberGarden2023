      ...
      {data.map((item, index) => (
        ...
        >
          <Col className={`${styles.col} position-relative`} xs={1}>
            ...
            <div className="ms-3">{item.visibleNumber}</div>
          </Col>
          <Col className={`${styles.col}`}>{item.operationName}</Col>
          <Col
            className={`${styles.col}`}
            xs={3}
          >{`${item.departureStation.title}, Парк: ${item.departurePark.name} (${item.departureWay.name})`}</Col>
          <Col
            className={`${styles.col}`}
            xs={3}
          >{`${item.destinationStation.title}, Парк: ${item.destinationPark.name} (${item.destinationWay.name})`}</Col>
          <Col className={`${styles.col}`} xs={1}>
            {item.normalMinutesLength}
          </Col>
